...
use katherine_engine::persistence::neuro_persist::PersistNeuro;
    ...
    let hub: Arc<dyn katherine_core::hub::Hub> =
        match katherine_engine::memory::libsql_store::LibSqlHub::new(db_path) {
            Ok(store) => {
                ...
                if report.total > 0 {
                    eprintln!(
                        "mem: {} total, {} superseded, {} decayed, {} archived",
                        report.total, report.superseded, report.decayed, report.archived
                    );
                }
                Arc::new(store)
            }
...
async fn setup() -> (
    ...
    Arc<ToolRegistry>,
    Arc<dyn katherine_core::neuro::Neuro>,
    CancellationToken,
    ...
    let (hub, neuro_observer) =
        match katherine_engine::memory::libsql_store::LibSqlHub::new_with_conn(db_path) {
            Ok((store, conn)) => {
                ...
                let mem_neuro = Arc::new(MemNeuro::new());
                let persist = PersistNeuro::from_arc(
                    mem_neuro.clone(),
                    &std::env::var("KATHERINE_HOME")
                        .map(std::path::PathBuf::from)
                        .unwrap_or_else(|_| std::path::PathBuf::from(".")),
                );
                let observer = katherine_engine::neuro_v3::NeuroObserver::spawn(conn, mem_neuro.clone());
                let neuro: Arc<dyn katherine_core::neuro::Neuro> = Arc::new(persist);
                (hub, (neuro, mem_neuro, Some(observer)))
            }
            Err(e) => {
                ...
                let mem_neuro = Arc::new(MemNeuro::new());
                let neuro: Arc<dyn katherine_core::neuro::Neuro> = Arc::new(
                    PersistNeuro::from_arc(mem_neuro.clone(), &std::path::PathBuf::from("."))
                );
                (hub, (neuro, mem_neuro, None))
            }
        };
    let neuro = neuro_observer.0;
    let _mem_neuro = neuro_observer.1;
    let observer = neuro_observer.2;
