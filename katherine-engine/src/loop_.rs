...
use crate::thinking::{self, ThinkingRecord};
...
pub fn run_loop(
    ...
    Box::pin(async_stream::stream! {
        ...
        // 认知档案——thinking 持久化（独立于记忆系统）
        let thinking_dir = std::env::var("KATHERINE_HOME")
            .map(std::path::PathBuf::from)
            .unwrap_or_else(|_| std::path::PathBuf::from("."))
            .join("thinking");
        let mut last_thinking_id: Option<String> = None;
        let mut had_correction_prev_turn = false;
        ...
        loop {
            ...
            // ── 调用模型 ────────────────────────────
            // 通讯层（provider）内部处理重试、退避、断路、流→非流降级。
            // loop 层只消费最终事件流。部分内容直接使用（方案 A）。
                        ...
                        match ev {
                            ...
                            Some(Err(e)) => {
                                ...
                                if let Some(ref tx) = config.neuro_tx {
                                    let _ = tx.send(NeuroEvent::ErrorOccurred {
                                        error_type: format!("{:?}", e),
                                        message: e.to_string(),
                                        turn,
                                    });
            ...
            if let Some(ref tx) = config.neuro_tx {
                let _ = tx.send(NeuroEvent::ApiCalled {
            ...
            if tool_blocks.is_empty() && text_buf.is_empty() {
                ...
                eprintln!(
                    "[empty_response] turn={turn} system_len={sys_len} msg_count={msg_count} stream_error={has_stream_err}",
            ...
            for tb in &tool_blocks {
                ...
                // 安全中间件：校验工具能力声明
                let action = tb.input.as_ref()
                    .and_then(|v| v.get("action"))
                    .and_then(|a| a.as_str());
                if let Err(sec_err) = SecurityMiddleware::check(
                    tool.as_ref(),
                    action,
                    &crate::security::capabilities_for_call(&tb.name, action),
                ) {
                    let err = ToolResult::error(sec_err.to_string());
                    yield AgentEvent::ToolResult {
                        tool_use_id: tb.id.clone(),
                        result: err.clone(),
                    };
                ...
                let neuro_tx_clone = config.neuro_tx.clone();
                ...
                join_set.spawn(async move {
                    ...
                    if let Some(ref tx) = neuro_tx_clone {
                        let _ = tx.send(NeuroEvent::ToolExecuted {
                            name: name.clone(),
