...
impl Tool for BashTool {
    fn definition(&self) -> ToolDefinition {
        ToolDefinition {
            name: "Bash".into(),
            description: "执行 Bash 命令并返回输出。在 Git Bash（POSIX sh）中运行，使用 Unix shell 语法（/dev/null 不是 NUL，正斜杠路径）。避免用 Bash 做有专用工具的事——Glob 搜文件名、Grep 搜内容、Read 读文件。Bash 留给真正的终端操作。".into(),
            input_schema: serde_json::json!({
                "type": "object",
                "properties": {
