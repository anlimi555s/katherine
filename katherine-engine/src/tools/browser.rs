...
/// 浏览器控制工具——Lazy init。
/// 内部持有持久化 tokio runtime，确保 browser handler 不会随 execute() 调用结束而销毁。
pub struct BrowserTool {
    chrome_path: PathBuf,
    /// (runtime, browser) —— runtime 必须和 browser handler 共存亡。
    /// 用 OnceLock 保证只启动一次。
    state: OnceLock<(tokio::runtime::Runtime, Arc<Browser>)>,
}
...
impl BrowserTool {
    ...
    /// 获取 browser 引用。首次调用时启动 Chrome + 持久化 runtime。
    /// 后续调用直接返回已缓存的 browser。
    fn get_or_init_browser(&self) -> Result<&Arc<Browser>, EngineError> {
        if let Some((_, browser)) = self.state.get() {
            return Ok(browser);
        }
        ...
        let rt = tokio::runtime::Builder::new_multi_thread()
            ...
            .enable_io()
            .worker_threads(2)
            .thread_name("browser-rt")
            .build()
            .map_err(|e| EngineError::Config(format!("runtime: {e}")))?;
        ...
        let (browser, mut handler) = rt
            ...
            .map_err(|e| EngineError::Config(format!("Launch failed: {e}")))?;

        // handler 必须在这个持久 runtime 上运行
        rt.spawn(async move {
            while let Some(_) = handler.next().await {}
        });

        let browser = Arc::new(browser);
        let _ = self.state.set((rt, browser.clone()));
        let (_, browser) = self.state.get().unwrap();
        Ok(browser)
    }
}
...
impl Tool for BrowserTool {
    fn definition(&self) -> ToolDefinition {
        ToolDefinition {
            name: "browser".into(),
            description: "通过 Chromium 浏览器访问和交互网页。支持导航、点击、输入、等待元素出现、截图（全页或按选择器）、提取可见文本/HTML、执行 JavaScript、管理 Cookie。用于：需要 JavaScript 渲染的 SPA 页面、需要登录态的操作、动态内容提取、访问本地服务页面（localhost/:9876）。不用 Browser 的情况：纯 REST API 调用用 Bash curl。首次使用时启动 Chrome（有头模式，操作过程可见）。".into(),
    ...
    fn execute(&self, input: serde_json::Value) -> Result<ToolResult, EngineError> {
        ...
        // 确保 browser 已初始化
        self.get_or_init_browser()?;
        let (rt, browser) = self.state.get().unwrap();
    ...
    fn capabilities(&self) -> Vec<Capability> {
        vec![Capability::NetOutbound, Capability::NetLocalhost, Capability::JsEvaluate, Capability::CookieRead, Capability::CookieWrite]
    }
}
...
fn check_url(url: &str) -> Result<(), EngineError> {
    ...
    if lower.starts_with("file://") {
        return Err(EngineError::ToolPermissionDenied { name: "browser".into(), message: None });
    }
    if !lower.starts_with("https://") && !lower.starts_with("http://") {
        return Err(EngineError::ToolInputValidation { name: "browser".into(), errors: vec!["URL must start with https:// or http://".into()] });
    }
    Ok(())
}
