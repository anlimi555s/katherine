...
impl Tool for MarkMemoryTool {
    fn definition(&self) -> ToolDefinition {
        ToolDefinition {
            ...
            description: "存重要对话到长期记忆。必须是 [Selena]/[Katherine] 原文格式，不做摘要、不改措辞。importance 默认 0.5，纠正/决策设 0.9。用于：保存 Selena 的纠正、架构决策、重要的上下文片段。不用 mark_memory 的情况：一般性对话和闲聊不需要存。".into(),
...
impl Tool for RecallTool {
    fn definition(&self) -> ToolDefinition {
        ToolDefinition {
            name: "recall".into(),
            description: "搜索长期记忆——用自然语言查相关对话片段，按相关度排序。用于：需要历史上下文时、Selena 问以前的事时、需要回顾之前的决策时。不用 recall 的情况：简单问候、闲聊、纯技术问题不需要查记忆。".into(),
            input_schema: serde_json::json!({
                "type": "object",
                "properties": {
...
impl Tool for SaveDecisionTool {
    fn definition(&self) -> ToolDefinition {
        ToolDefinition {
            ...
            description: "记录架构决策到长期记忆。自动标记 importance=0.9，说明决定内容和否决的方案。用于：技术选型、架构变更、设计取舍。和 mark_memory 的区别：save_decision 专门用于有明确否决方案的决策记录。".into(),
            input_schema: serde_json::json!({
                "type": "object",
                "properties": {
