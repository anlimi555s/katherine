...
impl LibSqlHub {
    ...
    pub async fn consolidate(&self) -> ConsolidationReport {
        ...
        if total == 0 {
            return ConsolidationReport { total: 0, decayed: 0, archived: 0 };
        }

        // 衰减：批量更新 importance（幂律、指数、对数在 SQL 里直接算）
        // 这里只做标记——真正的衰减在检索时算（P2）
        // consolidation 做的是清理工作：归档 importance < 0.01 的事件
        let archived = conn
            .execute(
                "UPDATE events SET importance = importance * 0.001 WHERE importance < 0.01 AND decay_curve != 'none'",
                [],
            )
            .unwrap_or(0);

        ConsolidationReport { total, decayed: 0, archived }
    }
}
