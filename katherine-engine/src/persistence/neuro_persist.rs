...
impl PersistNeuro {
    /// 创建并重放历史数据。内部创建新的 MemNeuro。
    pub fn open(state_dir: &Path) -> Self {
        Self::from_arc(Arc::new(MemNeuro::new()), state_dir)
    }

    /// 从已有的 Arc<MemNeuro> 创建——与 NeuroObserver 共享同一实例。
    pub fn from_arc(inner: Arc<MemNeuro>, state_dir: &Path) -> Self {
    ...
    /// 返回共享的 Arc<MemNeuro>（给 NeuroObserver 等）。
    pub fn inner_arc(&self) -> Arc<MemNeuro> {
        self.inner.clone()
    }
}
