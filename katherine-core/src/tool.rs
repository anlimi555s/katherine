...
pub trait Tool: Send + Sync {
    ...
    /// 返回工具声明的能力列表。
    /// SecurityMiddleware 在执行前校验：任何超出此列表的能力使用将被拦截。
    /// 默认返回空列表——deny-by-default。
    fn capabilities(&self) -> Vec<crate::capability::Capability> {
        vec![]
    }

    /// [废弃] 工具是否自己做了安全检查？
    /// 已被 capabilities() 替代。保留以维持编译兼容。
    /// 原有语义：返回 true → 跳过中央权限检查。
    /// 当前所有 Execute 工具都设了 true，导致中央检查为死代码。
    #[deprecated(since = "3.1", note = "Replaced by capabilities()")]
